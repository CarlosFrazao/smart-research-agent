...
def _make_result(
    url: str, source: str, title: str, desc: str, confidence: float = 0.8
) -> SearchResult:
...
def test_detect_confirms_relation():
    ...
    b = Claim(
        id="b1",
        text=claim_text,
        source="http://b.com",  # fonte diferente, mesmo conteúdo
        source_name="sourceB",
        confidence=0.8,
        tokens=graph._tokenize(claim_text),
    )
...
def test_no_self_source_relations():
    ...
    a = Claim(
        id="x1",
        text=text,
        source="http://same.com",
        source_name="same",
        confidence=0.9,
        tokens=graph._tokenize(text),
    )
    b = Claim(
        id="x2",
        text=text,
...
def test_export_graphviz():
    graph = EvidenceGraph()
    text = (
        "Studies show that AI adoption in enterprises increased by 45 percent in "
