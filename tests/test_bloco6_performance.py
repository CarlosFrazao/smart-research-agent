...
from src.search.semantic_reranker import SemanticReranker, _keyword_score
...
from src.types import SearchResult, ExpandedQuery, IntentResult
...
async def test_keyword_score_and_fallback_reranker():
    results = [
        {"title": "Outro assunto completamente diferente", "snippet": "Nada a ver com Python", "url": "http://other.com"},
        {"title": "Python Programming", "snippet": "Learn python programming, loops, classes", "url": "http://python.org"},
    ]
...
async def test_semantic_rerank_with_sentence_transformers():
    ...
    results = [
        {"title": "Outro assunto", "snippet": "Abacaxis e laranjas no pomar", "url": "http://fruits.com"},
        {"title": "Desenvolvimento Web", "snippet": "Criando aplicações modernas com React, HTML e CSS", "url": "http://react.dev"},
    ]
...
async def test_reasoning_service_rank_integration():
    ...
    results = [
        ...
        MockResult("Python", "Escrever código Python de alta performance", "http://python.org"),
    ]
...
async def test_serpapi_searcher_mocked_results():
    with patch("src.search.serpapi_searcher._SerpAPIGoogleSearch") as mock_search_class, \
         patch("src.search.serpapi_searcher._SERPAPI_AVAILABLE", True):
        mock_instance = MagicMock()
        mock_instance.get_dict.return_value = {
            "organic_results": [
                {"title": "Resultado 1", "link": "http://res1.com", "snippet": "Snippet 1", "position": 1},
                {"title": "Resultado 2", "link": "http://res2.com", "snippet": "Snippet 2", "position": 2},
            ]
        }
...
async def test_search_service_serpapi_fallback():
    ...
    mock_serpapi.search = AsyncMock(return_value=[
        {"title": "SerpAPI Result", "url": "http://serpapi.com", "snippet": "SerpAPI Snippet"}
    ])
    
    mock_orch.searchers = {
        "searxng": mock_searcher_fail,
        "serpapi": mock_serpapi
    }
    ...
    class MockPlan:
        def __init__(self):
            self.sources = {"searxng": [ExpandedQuery(query="Python info", type="general", priority="alta", rationale="test")]}
    ...
    results = await svc.execute([ExpandedQuery(query="Python info", type="general", priority="alta", rationale="test")], MockPlan(), intent)
