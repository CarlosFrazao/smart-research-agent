...
def test_source_planner_ai_ml():
    from src.source_planner import SourcePlanner
    from src.types import Domain, Intention, ExpandedQuery
    ...
    query = ExpandedQuery(
        query="llm local", type="qualificador", priority="alta", rationale="test"
    )
    planner = SourcePlanner()
...
def test_source_planner_all_sources_have_queries(sample_intent, sample_expanded_query):
    ...
    queries = [
        ExpandedQuery(
            query=f"query {i}", type="qualificador", priority="alta", rationale="test"
        )
        for i in range(5)
    ]
    planner = SourcePlanner()
...
async def test_ranker_sorted_by_score():
    ...
    r1 = SearchResult(
        source="github",
        title="low/stars",
        url="https://github.com/low",
        description="",
        metrics={"stars": 10, "forks": 1},
    )
    r2 = SearchResult(
        source="github",
        title="high/stars",
        url="https://github.com/high",
        description="",
        metrics={"stars": 50000, "forks": 5000},
    )
...
async def test_synthesizer_clusters_by_entity():
    ...
    r2 = RankedResult(
        source="reddit",
        title="twenty crm review",
        url="https://reddit.com/t/t",
        description="Review",
        score=60.0,
    )
    r3 = RankedResult(
...
async def test_synthesizer_applies_source_cap():
    ...
    results = [
        RankedResult(
            source="github",
            title=f"repo{i}/project{i}",
            url=f"https://github.com/{i}/{i}",
