...
async def test_full_pipeline_with_mocks():
    ...
    # Mock do gerador LLM
    with patch("src.clients.llm_client.LLMClient.complete", new_callable=AsyncMock) as mock_complete, \
         patch("src.clients.llm_client.LLMClient.generate", new_callable=AsyncMock) as mock_generate, \
         patch("src.clients.llm_client.LLMClient.generate_structured", new_callable=AsyncMock) as mock_struct:
        ...
        async def mock_struct_impl(prompt, schema, *args, **kwargs):
            ...
            if "expanded_queries" in props:
                return {
                    "expanded_queries": ["Rust async practices best methods"]
                }
        ...
        with patch("src.services.search_service.SearchService.execute", new_callable=AsyncMock) as mock_search_exec:
            from src.types import SearchResult
            mock_search_exec.return_value = [
                SearchResult(source="test", title="Rust vs Python", url="https://example.com/rust", description="Rust is fast")
            ]
