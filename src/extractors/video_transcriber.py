...
class VideoTranscriber:
    ...
    def _get_model(self) -> object | None:
        ...
        try:
            ...
            logger.info(
                f"VideoTranscriber: modelo Whisper {self._model_name!r} carregado."
            )
        except Exception as exc:
            self._model = None
            logger.warning(
                f"VideoTranscriber: Whisper indisponivel ({exc}). Fallback ativo."
            )
        return self._model
    ...
    async def _download_audio(self, url: str) -> str | None:
        ...
        try:
            ...
            logger.warning(
                "VideoTranscriber: yt-dlp nao instalado. Instale com: pip install yt-dlp"
            )
            return None
    ...
    async def transcribe_url(self, url: str) -> str | None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                logger.warning(
                    f"VideoTranscriber.transcribe_url falhou em {url}: {exc}"
                )
        return None
