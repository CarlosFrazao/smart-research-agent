...
class SemanticCache:
    ...
    async def warm(self, queries: list[str] | None = None) -> int:
        ...
        try:
            logger.info(f"SemanticCache: Aquecendo cache com {len(queries_to_warm)} queries")
    ...
    def index(
        ...
        try:
            ...
            logger.info(f"SemanticCache síncrono INDEX: '{query_text[:50]}' [key={query}, ttl={effective_ttl}s]")
        except Exception as e:
