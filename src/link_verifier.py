    ...
    async def verify_results(self, results: list[SearchResult]) -> list[SearchResult]:
        ...
        for result in results:
            ...
            valid_links = [
                ...
                if not l.startswith("http://localhost")
                and not l.startswith("http://127.0.0.1")
            ]
        ...
        logger.info(
            f"LinkVerifier: Verificando {len(all_urls_to_verify)} URLs citadas concorrentemente..."
        )
        ...
        async with httpx.AsyncClient(headers=headers, verify=False) as client:
            ...
            for url, is_alive, status, err in verifications:
                ...
                if not is_alive:
                    logger.warning(
                        f"LinkVerifier: Link quebrado detectado: {url} (Status {status} / {err})"
                    )
        ...
        for result in results:
            ...
            for url in links:
                is_alive, status, err = url_status_map.get(
                    url, (False, 0, "Not Verified")
                )
                if is_alive:
            ...
            if dead_links:
                ...
                result.confidence_score = round(
                    max(0.0, result.confidence_score - penalty), 3
                )
                ...
                logger.info(
                    f"LinkVerifier: Penalizado {result.url[:50]} com -{penalty} por conter links quebrados."
                )
