"""Gerenciador de proxies residenciais e rotativos para contornar bloqueios anti-scraping."""
...
class ProxyManager:
    ...
    async def validate_all_proxies(self):
        ...
        self.proxies = [
            p for p in self.proxies if p.proxy_type == "vps_ipv6" or p.health_score > 50
        ]
        logging.info(
            f"Proxy validation complete. Active pool: {len(self.proxies)} / {original_len} proxies."
        )
    ...
    def get_best_proxy(self, target_domain: str) -> Proxy | None:
        ...
        candidates = [
            ...
            if p.health_score > 30 and target_domain not in p.blocked_domains
        ]
        if not candidates:
            return None
    ...
    async def tunnel_https(
        ...
        try:
            ...
            if b"200" in resp.split(b"\r\n")[0]:
                ...
                self.manager.report_result(
                    proxy, host, success=True, response_time=duration
                )
            else:
                logging.warning(
                    f"Proxy handshake failed for {proxy.get_url()}. Falling back to direct connection to {host}:{port}"
                )
                ...
                try:
                    ...
                    logging.error(
                        f"Fallback direct connection failed to {host}:{port}: {ex}"
                    )
                    client_writer.close()
                    remote_writer.close()
        except Exception as e:
            logging.warning(
                f"Failed tunnel via proxy {proxy.get_url()} to {host}:{port}: {e}. Falling back to direct connection."
            )
            self.manager.report_result(proxy, host, success=False)
            # Fallback Direto
            try:
