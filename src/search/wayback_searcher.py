"""Searcher usando o Wayback Machine (Internet Archive) para busca em versoes arquivadas de paginas."""
...
class WaybackSearcher(BaseSearcher):
    """Buscador especializado para coletar e estruturar resultados vindos do Wayback."""
    ...
    async def search(self, query: str, **kwargs) -> list[SearchResult]:
        """Realiza busca assíncrona por termos no Wayback.
        ...
        if not (query.startswith("http://") or query.startswith("https://")):
            logger.info(
                "WaybackSearcher: A query fornecida não é uma URL. Ignorando busca no Wayback Machine."
            )
            return []
        ...
        logger.info(
            f"WaybackSearcher: Consultando histórico de capturas para '{query}'..."
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code != 200:
                    logger.warning(
                        f"WaybackSearcher: Erro ao consultar API (HTTP {response.status_code})"
                    )
                ...
                if not archived or "closest" not in archived:
                    logger.info(
                        f"WaybackSearcher: Nenhuma captura arquivada encontrada para {query}"
                    )
                    return []

                closest = archived["closest"]
                logger.info(
                    f"WaybackSearcher: Captura mais próxima encontrada em {closest.get('timestamp')}"
                )
            ...
            logger.error(
                f"WaybackSearcher: Falha ao executar busca no Internet Archive: {e}"
            )
            return self.fallback(query)

    def normalize(self, raw_result: dict[str, Any]) -> SearchResult:
        """Normaliza um resultado bruto vindo do Wayback para a entidade padrão `SearchResult`.
