...
class CircuitBreaker:
    ...
    def state(self) -> CircuitState:
        if self._state is CircuitState.OPEN and self._opened_at is not None:
            ...
            if elapsed >= self.config.reset_timeout_seconds:
                logger.info("circuit_breaker_half_open name=%s after=%.1fs", self.name, elapsed)
    ...
    def on_failure(self) -> None:
        ...
        if self._state is CircuitState.HALF_OPEN:
            logger.warning("circuit_breaker_reopened name=%s (falha durante teste half-open)", self.name)
...
class BaseSearcher(ABC):
    ...
    def __init__(self, config: dict[str, Any] | None = None, **kwargs: Any):
        ...
        cb_success_threshold = config.get("circuit_breaker_half_open_success_threshold", 2)
    ...
    async def _http_request(
        ...
        *,
        ...
        raise SearcherError(f"[{self.name}] retry loop terminou sem resposta nem exceção.")
